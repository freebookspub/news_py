...
website = "https://montrealgazette.com"
...
def get_url(name):
    sql = """
        select * from news where id=1518 and source = %s
    """
...
def parse_content(name):
    ...
    for list in lists:
        try:
            ...
            body = soup.find("body")
            article = body.find('article', class_='article-content-story article-content-story--story')
            head = article.find('header', class_="article-header")
            if head:
                headfigure = head.findAll("source")
                if headfigure:
                    headimgs = getImg(headfigure)
            ...
            alltext = article.find('section', class_='article-content__content-group')
            for item in alltext:
                try:
                    if "p" == item.name:
            ...
            # print('111',list[0], headimgs, headalt, json.dumps(content))
